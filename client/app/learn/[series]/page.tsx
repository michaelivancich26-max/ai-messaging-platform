...
const COLOR = {
  red:     { text: "text-red-400",     bar: "bg-red-500",     ring: "ring-red-900/40",     bg: "bg-red-950/30",     pill: "bg-red-950/40 text-red-400"     },
  blue:    { text: "text-blue-400",    bar: "bg-blue-500",    ring: "ring-blue-900/40",    bg: "bg-blue-950/30",    pill: "bg-blue-950/40 text-blue-400"    },
  emerald: { text: "text-emerald-400", bar: "bg-emerald-500", ring: "ring-emerald-900/40", bg: "bg-emerald-950/30", pill: "bg-emerald-950/40 text-emerald-400" },
  amber:   { text: "text-amber-400",   bar: "bg-amber-500",   ring: "ring-amber-900/40",   bg: "bg-amber-950/30",   pill: "bg-amber-950/40 text-amber-400"   },
} as const;
...
export default function SeriesPage() {
  ...
  if (!series) return <div className="flex h-full items-center justify-center bg-gray-950 text-gray-500 text-sm">Series not found.</div>;
    ...
    <div className="min-h-full bg-gray-950">
      ...
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-gray-800 bg-gray-950/95 px-4 py-3 backdrop-blur-sm pt-safe">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <svg viewBox="0 0 16 16" fill="currentColor" className="h-3 w-3 text-gray-700 shrink-0"><path fillRule="evenodd" d="M6.22 4.22a.75.75 0 0 1 1.06 0l3.25 3.25a.75.75 0 0 1 0 1.06l-3.25 3.25a.75.75 0 0 1-1.06-1.06L8.94 8 6.22 5.28a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" /></svg>
          <span className="text-sm font-semibold text-gray-100 truncate">{series.title}</span>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-2xl px-4 py-8 space-y-6">
        ...
        <div className={`rounded-2xl border ${c.ring} ${c.bg} p-5 space-y-3`}>
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400 leading-relaxed">{series.description}</p>
          </div>
        ...
        <div className="space-y-2">
          {series.lessons.map((lesson, idx) => {
            ...
            return (
              ...
              >
                {/* Number / check */}
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold transition-colors ${isDone ? "bg-emerald-500 text-white" : `border-2 border-gray-700 text-gray-500 group-hover:border-gray-500`}`}>
                  {isDone ? (
                    <svg viewBox="0 0 16 16" fill="currentColor" className="h-4 w-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-100 leading-tight">{lesson.title}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <svg viewBox="0 0 16 16" fill="currentColor" className="h-3.5 w-3.5 text-gray-700 group-hover:text-gray-500 transition-colors">

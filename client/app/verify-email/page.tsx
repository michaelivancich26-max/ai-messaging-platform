  ...
  if (status === "success") {
    return (
      <div className="text-center space-y-4">
        ...
        <h2 className="text-lg font-semibold text-gray-100">Email verified!</h2>
        <p className="text-sm text-gray-400">Your email address has been confirmed. You're all set.</p>
        <button
  ...
  return (
    <div className="text-center space-y-4">
      <div className="flex h-14 w-14 items-center justify-center rounded-full bg-indigo-500/20 mx-auto">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-7 w-7 text-indigo-400">
      ...
      {resendDone ? (
        <p className="text-sm text-emerald-400">Email resent — check your inbox.</p>
      ) : (
        <>
          {resendError && <p className="text-xs text-red-400">{resendError}</p>}
          <button
            onClick={handleResend}
            disabled={resending}
            className="text-sm text-indigo-400 hover:text-indigo-300 underline underline-offset-2 disabled:opacity-50 transition-colors"
          >
            {resending ? "Sending…" : "Resend verification email"}
          </button>
        </>
...
export default function VerifyEmailPage() {
  return (
    <main className="flex min-h-screen items-center justify-center px-4 bg-gray-950">

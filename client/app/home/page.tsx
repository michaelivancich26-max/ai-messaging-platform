...
const MODES: { href: string; label: string; blurb: string; accent: string; icon: React.ReactNode }[] = [
  { href: "/lobby", label: "Common Grounds", blurb: "Join live rooms", accent: "text-indigo-400",
  ...
  { href: "/compete", label: "Battle Grounds", blurb: "1v1, AI judged", accent: "text-violet-400",
  ...
  { href: "/arena", label: "Training Grounds", blurb: "Beat the bots", accent: "text-amber-400",
  ...
  { href: "/learn", label: "Learn", blurb: "Debate theory", accent: "text-teal-400",
  ...
  return (
    <div className="h-full overflow-y-auto bg-gray-950">
      <div className="mx-auto max-w-4xl px-4 py-6 md:py-8 space-y-8">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-100">Welcome back{username ? `, ${username}` : ""}</h1>
        ...
        <section>
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-300">Live now</h2>
            <span className="text-xs text-gray-600">— bet the board</span>
          </div>
        ...
        {openBets.length > 0 && (
          <section>
            <h2 className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-gray-500">
              <GavelIcon className="h-3.5 w-3.5 text-amber-400" /> Your open bets
            </h2>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {MODES.map(m => (
              <button key={m.href} onClick={() => router.push(m.href)}
                className="flex flex-col items-start gap-2 rounded-2xl border border-gray-800 bg-gray-900/70 p-4 text-left transition-colors hover:border-gray-700 hover:bg-gray-900">
                <span className={m.accent}>{m.icon}</span>
                <div>
                  <div className="text-sm font-semibold text-gray-100">{m.label}</div>

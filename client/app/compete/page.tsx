  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {/* Stance */}
        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mt-4 mb-1.5">You are arguing</label>
              ...
              className={`flex-1 rounded-lg border py-2 text-xs font-semibold transition-colors ${
                stance === s
                  ? s === "affirmative"
                    ...
                    : "border-red-700 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300"
        ...
        {/* Win condition */}
        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mt-4 mb-1.5">Win condition</label>
          ...
          {(["exchanges", "time", "proposition"] as const).map(t => (
            <button
              ...
              className={`flex-1 rounded-lg border py-2 text-xs font-semibold capitalize transition-colors ${
                wcType === t ? "border-violet-600 bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300" : "border-gray-300 dark:border-gray-700 text-gray-500 hover:border-gray-300 dark:hover:border-gray-600"
              }`}
            >
              {t}
            </button>
        ...
        <div className="mt-6 flex gap-3">
          <button onClick={onClose} className="flex-1 rounded-xl border border-gray-300 dark:border-gray-700 py-2.5 text-sm text-gray-600 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">
            Cancel
          </button>
          <button
            onClick={submit}
...
function ChallengeCard({
  ...
  const stanceColor = challenge.stance === "affirmative"
    ? "border-emerald-800 bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400"
    : "border-red-800 bg-red-100 dark:bg-red-950/30 text-red-600 dark:text-red-400";

  return (
    <div className="group flex flex-col gap-3 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/60 dark:bg-gray-900/60 p-5 transition-all hover:border-gray-300 dark:hover:border-gray-700 hover:bg-white dark:hover:bg-gray-900">
      {/* Claim */}
      <p className="text-sm font-medium leading-relaxed text-gray-900 dark:text-gray-100">"{challenge.claim}"</p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {wc && (
          <span className="rounded border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 text-[10px] text-gray-600 dark:text-gray-400">
      ...
      {isMine ? (
        challenge.status === "open" && onCancel && (
          <button
            ...
            className="self-end rounded-lg border border-gray-300 dark:border-gray-700 px-3 py-1.5 text-xs text-gray-500 hover:border-red-800 hover:text-red-600 dark:hover:text-red-400 transition-colors"
          >
            Withdraw
          </button>
        )
...
export default function CompetePage() {
  ...
  return (
    <div className="flex h-full flex-col bg-gray-50 dark:bg-gray-950 text-white">
      ...
      {notification && (
        <div className="flex items-center gap-3 bg-violet-100 dark:bg-violet-900/80 border-b border-violet-700 px-4 py-3 text-sm">
          ...
          <button onClick={() => setNotification(null)} className="text-violet-700 dark:text-violet-300 hover:text-white">✕</button>
        </div>
      )}
                ...
                <div key={c.id}>
                  <div className="mb-1 flex items-center gap-2">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${
                      c.status === "open" ? "bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400" :
                      c.status === "matched" ? "bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400" :
              ...
              {leaderboard.length === 0 ? (
                ...
                leaderboard.map((entry, i) => {
                  ...
                  return (
                    <div
                      key={entry.id}
                      className={`flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-800 last:border-0 ${isMe ? "bg-violet-100 dark:bg-violet-950/30" : ""}`}
                    >
                      ...
                      <button
                        ...
                        className={`flex-1 truncate text-left text-sm font-medium hover:underline ${isMe ? "text-violet-700 dark:text-violet-300" : "text-gray-800 dark:text-gray-200"}`}
                      >
                      ...
                      {liveByUser.has(entry.id) && (
                        <button
                          ...
                          className="flex items-center gap-1 rounded-full bg-red-600/90 px-2 py-0.5 text-[10px] font-bold text-white hover:bg-red-500"
                        >

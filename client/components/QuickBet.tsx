...
export default function QuickBet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm rounded-t-2xl sm:rounded-2xl border border-gray-700 bg-gray-900 shadow-xl p-4 space-y-3" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-amber-300"><GavelIcon className="h-4 w-4" /> Place a bet</h2>
        ...
        <div className="flex gap-1.5">
          {CHIPS.map(c => (
            <button key={c} onClick={() => setAmount(c)}
              className={`flex-1 rounded-lg py-1.5 text-xs font-semibold transition-colors ${amount === c ? "bg-indigo-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}>
              {c}
            </button>
          ))}
          <input type="number" min={1} value={amount}
            onChange={e => setAmount(Math.max(1, Math.floor(Number(e.target.value) || 0)))}
            className="w-20 rounded-lg bg-gray-800 px-2 py-1.5 text-sm text-gray-100 outline-none ring-1 ring-gray-700 focus:ring-indigo-500" />
        </div>

    ...
    <div className="fixed inset-0 z-40 flex justify-end" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-gray-800 px-4 py-3 shrink-0">
          <span className="font-semibold text-gray-100">Room details</span>
        ...
        <div className="border-b border-gray-800 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-base font-semibold text-gray-100">#{meta?.name ?? roomId}</span>
            {meta?.isPrivate && (
          ...
          {meta?.maxMembers && <p className="mt-1 text-xs text-gray-600">Max {meta.maxMembers} members</p>}
        </div>
              ...
              {onlineMembers.map((m) => (
                <li key={m.userId} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gray-800 text-xs font-bold text-gray-300">
                      {m.username[0].toUpperCase()}
                    </span>
                    <span className="text-sm text-gray-200">{m.username}</span>
                    {m.userId === currentUserId && <span className="text-xs text-gray-600">(you)</span>}
                  </div>
        ...
        {canEdit && (
          <div className="px-4 py-4 space-y-4">
            ...
            {/* Max members */}
            <div>
              <label className="mb-1 block text-xs text-gray-400">Max members</label>
              <input
                type="number"
                value={editMax}
            ...
            {/* Private toggle */}
            <div className="flex items-center justify-between rounded-xl bg-gray-800/60 px-3 py-2.5">
              <div>
            ...
            {editPrivate && (
              <div>
                <label className="mb-1 block text-xs text-gray-400">
                  {meta?.isPrivate ? "Change password" : "Set password"} <span className={meta?.isPrivate ? "text-gray-600" : "text-red-400"}>

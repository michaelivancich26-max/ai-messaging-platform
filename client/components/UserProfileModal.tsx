  ...
  return (
    <div className="rounded-xl bg-gray-800/60 ring-1 ring-gray-700/40 p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className="flex items-center gap-1 rounded-full bg-emerald-900/30 px-2 py-0.5 text-[10px] text-emerald-400 ring-1 ring-emerald-700/30">
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{cred.supported} supported
        </span>
        <span className="flex items-center gap-1 rounded-full bg-red-900/30 px-2 py-0.5 text-[10px] text-red-400 ring-1 ring-red-700/30">
          <span className="h-1.5 w-1.5 rounded-full bg-red-500" />{cred.refuted} refuted
        </span>
        <span className="flex items-center gap-1 rounded-full bg-gray-800 px-2 py-0.5 text-[10px] text-gray-500 ring-1 ring-gray-700/30">
          <span className="h-1.5 w-1.5 rounded-full bg-gray-500" />{cred.contested} contested
        </span>
      </div>
    </div>
  );
...
export default function UserProfileModal({ userId, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-sm mx-4 rounded-2xl bg-gray-900 ring-1 ring-gray-700/60 shadow-2xl overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Close */}
        <button
          onClick={onClose}
          className="absolute right-3 top-3 rounded-full p-1.5 text-gray-500 hover:bg-gray-800 hover:text-gray-300 transition-colors"
        >
        ...
        {loading ? (
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 shrink-0 overflow-hidden rounded-full ring-2 ring-gray-700">
                {profile.avatarUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={profile.avatarUrl} alt={profile.username} className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center bg-gray-800 text-xl font-bold text-gray-400">
                    {profile.username[0]?.toUpperCase()}
                  </div>
                )}
              </div>
              <div className="min-w-0">
                <p className="text-base font-semibold text-gray-100 truncate">{profile.username}</p>
            ...
            {!profile.cred && (
              <div className="rounded-xl bg-gray-800/40 ring-1 ring-gray-700/30 px-4 py-3 text-xs text-gray-600 text-center">

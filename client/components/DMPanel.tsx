...
export default function DMPanel({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-sm rounded-t-2xl sm:rounded-2xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-xl flex flex-col max-h-[75vh]"
      ...
      >
        ...
        <div className="px-4 py-2.5 border-b border-gray-200 dark:border-gray-800 shrink-0">
          <div className="relative">
            <svg viewBox="0 0 20 20" fill="currentColor" className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-500 dark:text-gray-600 pointer-events-none">
            ...
            <input
              ...
              className="w-full rounded-lg bg-gray-100 dark:bg-gray-800 pl-8 pr-3 py-1.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-600 outline-none ring-1 ring-gray-300 dark:ring-gray-700 focus:ring-indigo-500"
            />
          </div>
        </div>
          ...
          {showSearch ? (
            results.length === 0 ? (
              ...
              <button key={u.id} onClick={() => openDM(u.id)} disabled={opening === u.id}
                className="flex w-full items-center gap-3 px-4 py-2.5 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-left">
                {u.avatarUrl
                  ? <img src={u.avatarUrl} alt={u.username} className="h-8 w-8 rounded-full object-cover shrink-0" />
                  : <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-violet-500/20 text-sm font-bold text-violet-600 dark:text-violet-400">{u.username[0].toUpperCase()}</span>
                }
                <span className="text-sm text-gray-800 dark:text-gray-200">{u.username}</span>
                {opening === u.id && <span className="ml-auto text-xs text-gray-500 dark:text-gray-600">Opening…</span>}
              </button>
            ))
          ) : dms.length === 0 ? (
            <p className="px-4 py-3 text-xs text-gray-500 dark:text-gray-600">No conversations yet. Search for a user to start one.</p>
          ) : dms.map(dm => {
            ...
            return (
              <button key={dm.name} onClick={() => { onClose(); router.push(`/room/${dm.name}`); }}
                ...
                <span className="text-sm text-gray-800 dark:text-gray-200">{partner.username}</span>
              </button>
            );
          })}
        </div>

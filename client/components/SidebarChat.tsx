...
export default function SidebarChat({ messages, currentUsername, onSend, onClose, variant = "side", readOnly = false }: Props) {
  ...
  const accentClass = isSpectator ? "bg-cyan-500" : "bg-gray-500";
    ...
    <div className="flex w-full md:w-72 shrink-0 flex-col border-l border-gray-800 bg-gray-950">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-gray-800 px-3 py-2.5">
        <div className="flex items-center gap-1.5 flex-1 min-w-0">
          ...
          <span className="ml-1 text-[10px] text-gray-600 truncate">
      ...
      <div className="pb-safe shrink-0 border-t border-gray-800 px-2 py-2">
        {readOnly ? (
          <p className="px-2 py-1.5 text-[10px] text-gray-600 text-center">
            Only spectators can write here
          </p>
        ) : (
          <div className={`flex items-center gap-1.5 rounded-lg bg-gray-800 px-2.5 py-1.5 ring-1 ring-gray-700 ${ringFocus} transition-colors`}>
            <input
              ref={inputRef}
              type="text"

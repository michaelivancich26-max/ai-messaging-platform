    ...
    <div className="relative" ref={panelRef}>
      <button
        ...
        className={`relative flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors ${
          open ? "bg-gray-700 text-gray-200" : "text-gray-400 hover:bg-gray-800 hover:text-gray-200"
        }`}
        title="Notifications"
      >
      ...
      {open && (
        <div className={`absolute z-50 rounded-xl border border-gray-700 bg-gray-900 shadow-2xl ${
            ...
            {pending.length > 0 && (
              <div className="border-b border-gray-800/60">
                {pending.map(n => {
                  ...
                  return (
                    <div key={n.id} className={`px-4 py-3 ${!n.read ? "bg-indigo-950/20" : ""}`}>
                      <div className="flex items-start gap-2.5">
                        <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-indigo-600/20 text-indigo-400">
                          ...
                          <div className="mt-2 flex gap-2">
                            ...
                            {!isDM && (
                              <button
                                ...
                                className="rounded-full border border-gray-700 px-3 py-0.5 text-[11px] font-semibold text-gray-400 hover:border-gray-600 hover:text-gray-300 transition-colors"
                              >
                                Decline
                              </button>
                            )}
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-200">
                      <span className="font-semibold">{n.fromUsername}</span>
                      {" mentioned you in "}
                      <span className="font-semibold text-indigo-400">#{n.roomName}</span>
                    </p>
                    {n.content && (
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="h-3 w-3 shrink-0 text-gray-600 mt-1">
              ...
              return (
                <div key={n.id} className="px-4 py-2.5 opacity-50">
                  <p className="text-xs text-gray-400">
                    ...
                    {" · "}
                    <span className={n.accepted ? "text-emerald-400" : "text-gray-500"}>

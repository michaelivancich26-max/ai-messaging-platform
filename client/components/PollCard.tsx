...
export default function PollCard({ poll, currentUserId, canClose, onVote, onClose }: Props) {
  ...
  return (
    <div className="mx-4 my-2 rounded-xl border border-gray-700/60 bg-gray-900 p-4 shadow-sm">
      {/* Header */}
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <p className="text-sm font-semibold text-gray-100">{poll.question}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {isClosed
            ? <span className="rounded-full bg-gray-700 px-2 py-0.5 text-[10px] text-gray-400">Closed</span>
            : <span className="rounded-full bg-indigo-500/20 px-2 py-0.5 text-[10px] text-indigo-300">Live</span>
          }
          {!isClosed && canClose && (
              ...
              <button
                ...
                className={`relative w-full overflow-hidden rounded-lg border text-left transition-colors
                  ${isMyVote
                    ...
                    : "border-gray-700 bg-gray-800/50 hover:border-gray-600 disabled:hover:border-gray-700"
                  }`}
              >
                {/* Fill bar */}
                <div
                  className={`absolute inset-y-0 left-0 transition-all duration-500 rounded-lg
                    ${isWinner ? "bg-indigo-500/20" : isMyVote ? "bg-indigo-500/10" : "bg-gray-700/40"}`}
                  style={{ width: `${pct}%` }}
                />
                <div className="relative flex items-center justify-between px-3 py-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm ${isMyVote ? "text-indigo-200 font-medium" : "text-gray-300"}`}>
                      {opt}
                    </span>
                    {isWinner && (
                      <span className="rounded-full bg-indigo-500/20 px-1.5 py-0.5 text-[10px] text-indigo-300">winner</span>
                    )}
                  </div>

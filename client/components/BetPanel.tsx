...
export default function BetPanel({ roomName, userId }: { roomName: string; userId: string }) {
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-gray-900 ring-1 ring-gray-200 dark:ring-gray-800 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="rounded-md bg-amber-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">Live bet</span>
      ...
      <div>
        ...
        <div className="mt-1 flex items-center justify-between text-[11px]">
          <span className="font-semibold text-emerald-700 dark:text-emerald-300">{market.labelA} · {pct(market.priceA)}</span>
          <span className="font-semibold text-rose-700 dark:text-rose-300">{pct(market.priceB)} · {market.labelB}</span>
        </div>
      </div>

      {settled ? (
        <div className="rounded-xl bg-gray-100/60 dark:bg-gray-800/60 p-3 text-center text-sm">
          {market.winningSide === null
            ? <span className="text-gray-700 dark:text-gray-300">Too close to call — bets refunded.</span>
            : <span className="text-gray-800 dark:text-gray-200"><b className={market.winningSide === "A" ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}>{labelOf(market.winningSide)}</b> took it.</span>}
        </div>
      ) : market.isParticipant ? (
        <p className="rounded-xl bg-gray-100/60 dark:bg-gray-800/60 p-3 text-center text-xs text-gray-600 dark:text-gray-400">You&apos;re debating this match — betting is disabled for participants.</p>
      ) : (
        <>
          {/* Side + amount + buy */}
          ...
          <div className="flex items-center gap-2">
            <input type="number" min={1} value={amount}
              ...
              className="w-24 rounded-lg bg-gray-100 dark:bg-gray-800 px-2.5 py-1.5 text-sm text-gray-900 dark:text-gray-100 outline-none ring-1 ring-gray-300 dark:ring-gray-700 focus:ring-indigo-500" />
      ...
      {/* My positions */}
      {positions.length > 0 && (
        <div className="space-y-1.5 border-t border-gray-200 dark:border-gray-800 pt-2.5">
          ...
          {positions.map((p) => {
            ...
            return (
              <div key={p.side} className="flex items-center justify-between gap-2 text-xs">
                <span className={p.side === "A" ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}>
                  {p.shares.toFixed(0)} × {labelOf(p.side)}
                </span>
                <span className="text-gray-600 dark:text-gray-400">
                  {money(value)} <span className={pnl >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>({pnl >= 0 ? "+" : ""}{money(pnl)})</span>
                </span>
                {!settled && (
                  <button onClick={() => trade("sell", p.side)} disabled={busy}
                    className="rounded-md bg-gray-100 dark:bg-gray-800 px-2 py-0.5 text-[11px] font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-40">
                    Cash out
                  </button>
                )}
              </div>
